...
class ParallelRuleChecker
  def initialize(rule_files, report, rule_binding, num_workers: 4)
  ...
  def run
    parallelize_with_processes(@rule_files)
    
    # Step 4: Return or process results
    #results
  end
  ...
  require 'io/wait'
  def parallelize_with_processes(rule_files)
    results = []
    pids = []
    ...
    @num_workers.times do |i|
      ...
      pid = fork do
        ...
        loop do
          ...
          # Do work
          puts "Worker #{i}: Processing #{File.basename(task)}"
          begin
            result = execute_rule(task)
            chunk_results << result
          rescue => e
            puts "Worker #{i}: Error processing #{File.basename(task)} : #{e.message}"
            
            chunk_results << { file: task, error: e.message }
          end
          puts "Worker #{i}: Done processing #{File.basename(task)}"
          # Tell master we are ready
          puppet_to_master_w.puts "ready"
    ...
    puts "All workers completed. Total results: #{results.size}"
    
    aggregated_res = RBA::ReportDatabase.new()
    results.each do | element |
      incoming_rep = RBA::ReportDatabase.new()
      incoming_rep.load_from_string(element[:result])
  ...
  def execute_rule(file)
    # Create isolated binding for each rule file
    # This prevents rules from interfering with each other
    ...
    # Get the result (assumes each rule sets a 'result' variable)
    result_string = @report.rdb.write_to_string()
    {
